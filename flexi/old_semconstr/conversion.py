...
from flexi.old_semconstr.logic import QLF, Apply, Lambda, NewVar, Iff, DRS, Typ, And, Const, NamedVar, Seq, Var, TRUE, HOL, \
    HolConst, HolApply, HolLambda
...
def qlf_to_hol(qlf: QLF) -> HOL:
    if isinstance(qlf, Var) or isinstance(qlf, Const):
        return HolConst(str(qlf), qlf.typ)
    elif isinstance(qlf, Apply):
        if isinstance(qlf.func, Lambda):
            return qlf_to_hol(qlf.force_single_beta_reduction())
        return HolApply(qlf_to_hol(qlf.func), qlf_to_hol(qlf.arg))
    elif isinstance(qlf, Lambda):
        v = qlf_to_hol(qlf.var)
        assert isinstance(v, HolConst)
        return HolLambda(v, qlf_to_hol(qlf.body))
    pass
