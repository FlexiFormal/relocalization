...
from examples.simplifier import RuleType, Simplifier
...
with wb.new_workbench(result_file, 'Parsing example sentences'):
    for input_file in [
        ...
        TEST_FILE_DIR / 'ftml' / 'adj-var-reduction.en.html',
        ...
        for i, sentence in enumerate(grammar.parse_ftml_to_sentences(input_file), start=1):
            ...
            for mast in sentence:
                # original sentence ast
                wb.push_sentence_mast(mast)

                modified_mast = simplifier.simplify(mast, type=RuleType.R2_1)

                # wb.push_sentence_mast(modified_mast)
